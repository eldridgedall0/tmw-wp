 ...
 * Trusted device interactions on the profile and login pages.
 */
(function ($) {
    'use strict';
    ...
    $(document).on('click', '.tmw-revoke-device-btn', function () {
        ...
        var $btn     = $(this).addClass('tmw-btn-loading').text(cfg.i18n.removing);
        var deviceId = $btn.data('device-id');

        $.post(cfg.ajaxurl, { action: 'tmw_core_revoke_device', nonce: cfg.nonce, device_id: deviceId })
            .done(function (res) {
                if (res.success) {
                    var $row = $btn.closest('.tmw-device-row');
                    $row.fadeOut(300, function () {
                        $row.remove();
                        if ($('.tmw-device-row').length === 0) { location.reload(); }
                    });
                } else {
    ...
    $(document).on('click', '#tmw-revoke-all-devices-btn', function () {
        ...
        $.post(cfg.ajaxurl, { action: 'tmw_core_revoke_all_devices', nonce: cfg.nonce })
            .done(function (res) {
            ...
            .fail(function () { alert(cfg.i18n.error); $btn.removeClass('tmw-btn-loading'); });
    });

    // =========================================================================
    // PROFILE — Trust this device
    // =========================================================================
    $(document).on('click', '#tmw-trust-this-device-btn', function () {
        var $btn = $(this).addClass('tmw-btn-loading').text(cfg.i18n.trusting);

        $.post(cfg.ajaxurl, { action: 'tmw_core_trust_device', nonce: cfg.nonce })
            .done(function (res) {
                if (res.success) {
    ...
    // =========================================================================
    // LOGIN — inject trust_device flag into the theme's AJAX call
    //
    // The theme's login JS calls $.ajax / $.post with action=tmw_login.
    // We patch $.ajax so that when the trust_device checkbox is checked,
    // the flag is appended to the POST body before it leaves the browser.
    // =========================================================================
    if ($('#tmw-login-form').length) {
        var _origAjax = $.ajax;
        $.ajax = function (settings) {
            if (
                settings &&
                typeof settings.data === 'string' &&
                settings.data.indexOf('action=tmw_login') !== -1 &&
                $('#trust_device').is(':checked')
            ) {
                settings.data += '&trust_device=true';
            }
