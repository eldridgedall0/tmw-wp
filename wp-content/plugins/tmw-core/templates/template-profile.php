 ...
 *
 * PATCHED BY TMW CORE v1.0.0
 * Change from original: do_action('tmw_profile_sections', $user) added
 * immediately before the Sign Out button. Everything else is unchanged.
 * TMW Core hooks in here to render Trusted Devices and Login Activity.
 */
...
<div class="tmw-profile-page">
    <div class="tmw-container tmw-container-narrow">
        ...
        <div class="tmw-profile-section">
            ...
            <div class="tmw-subscription-card">
                <div class="tmw-subscription-header">
                    <span class="tmw-subscription-tier"><?php echo esc_html( $level_name ); ?></span>
                    <?php if ( ! $is_free ) : ?>
                ...
                <div class="tmw-subscription-actions">
                    <?php if ( $is_free ) : ?>
                        <a href="<?php echo esc_url( tmw_get_page_url( 'subscription' ) ); ?>" class="tmw-btn tmw-btn-primary">
                            <i class="fas fa-arrow-up"></i> <?php _e( 'Upgrade Plan', 'flavor-starter-flavor' ); ?>
                        </a>
                    <?php elseif ( tmw_is_stripe_active() ) : ?>
                        <button type="button" class="tmw-btn tmw-btn-secondary tmw-stripe-portal-btn">
                            <i class="fas fa-cog"></i> <?php _e( 'Manage Subscription', 'flavor-starter-flavor' ); ?>
                        </button>
                    <?php else : ?>
                    ...
                    <a href="<?php echo esc_url( tmw_get_app_url() ); ?>" class="tmw-btn tmw-btn-ghost">
                        <i class="fas fa-rocket"></i> <?php _e( 'Go to App', 'flavor-starter-flavor' ); ?>
                    </a>
                </div>
            </div>
        </div>
        ...
        <?php
        /**
         * tmw_profile_sections
         *
         * Plugins hook here to inject additional profile sections.
         * TMW Core renders: Trusted Devices, Recent Login Activity.
         * The hook fires just before the Sign Out button so plugin sections
         * always appear at the bottom of the profile, above logout.
         *
         * @param WP_User $user  Current user object.
         */
        do_action( 'tmw_profile_sections', $user );
        ?>
